...
`{ id, order, tradition, school, author, source, translator, editionYear, reference, title, text, quote, primaryTheme, themes: string[], wordCount, lastShownDate: "YYYY-MM-DD"|null, timesShown, addedAt }` — alleen `source` en `text` zijn verplicht bij import, de rest is optionele metadata die getoond wordt als aanwezig. `order` bepaalt de gelijke-stand-volgorde voor nog niet getoonde teksten (zie "Volgorde en clustering" hieronder); `lastShownDate`/`timesShown` sturen de rotatie en worden nooit door een latere import of hershuffel overschreven van bestaande teksten.
...
**Volgorde en clustering** (`clusterShuffleOrder()` in `model.js`, `reshuffleOrder()` in `storage.js`): de kale bestandsvolgorde van een importpakket staat meestal per bron gegroepeerd (bv. alle hoofdstukken van Matthéüs achter elkaar), wat als eentonig aanvoelt. `order` wordt daarom niet als bestandsvolgorde bewaard, maar geclusterd op `primaryTheme`: eerst worden de themaclusters zelf in willekeurige volgorde gezet, en binnen elk cluster de teksten ook, zodat opeenvolgende dagen enige tijd rond hetzelfde thema blijven (met verschillende tradities/bronnen door elkaar) voordat het cluster wisselt. Dit gebeurt automatisch één keer direct na de eerste vulling van de bibliotheek (`seedFromBundledFileIfEmpty`), en is daarna via "Volgorde husselen" in de bibliotheek handmatig opnieuw te doen (bv. na een latere import). Raakt nooit `lastShownDate` aan: alleen de gelijke-stand-volgorde voor nog niet (of het langst geleden) getoonde teksten verandert, nooit de al opgebouwde geschiedenis.
